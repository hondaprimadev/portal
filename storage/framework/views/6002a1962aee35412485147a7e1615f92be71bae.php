<script type="text/javascript">
	...
	var myDropzone = new Dropzone("div#UploadMemo", { 
    	...
    	url: "<?php echo e(route('memo.upload.post')); ?>",
	...
	function getSupplier(name, branch, edit=''){
		...
		if (name) {
			$.ajax({
				url: '<?php echo e(route('memo.supplier.all')); ?>?s='+name+'&b='+branch,
				type: 'GET',
				data:{
					'_token': tokenId,
				},
	...
	function getSupplierId(s_id, t_id) {
		var tokenId = $('input[name=_token]').val();
		if(s_id > 0){
			console.log('<?php echo e(route('memo.supplier.get')); ?>?id='+s_id+'&type='+t_id)
			$.ajax({
				url: '<?php echo e(route('memo.supplier.get')); ?>?id='+s_id+'&type='+t_id,
				type: 'GET',
				data: {
					'_token': tokenId,
				},
