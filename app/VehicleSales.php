...
class VehicleSales extends Model
{
    ...
    public function scopeOfTableAll($query, $begin, $tahun_m1, $bulan_m1, $branch='')
    {
        $query->select(
            ...
            DB::raw('sum(case when position_id="B5MK" AND DATE_FORMAT(faktur_date,"%Y-%m")="'.$begin.'" then active else 0 end) as total_spv'),
            DB::raw('sum(case when position_id="B5" AND DATE_FORMAT(faktur_date,"%Y-%m")="'.$begin.'" then active else 0 end) as total_bm'),
    ...
    public function scopeOfTableBranch($query, $begin, $tahun_m1, $bulan_m1, $b='')
    {
        $query->select(
            ...
            DB::raw('sum(case when vehicle_sales.position_id="B5" AND DATE_FORMAT(faktur_date,"%Y-%m")="'.$begin.'" then active else 0 end) as total_bm'),
